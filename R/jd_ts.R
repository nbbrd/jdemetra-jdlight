...
jd_airline<-function(len, freq=12, startdate=c(2000,1),th=-.6, bth=-.8, fixed=TRUE){
  factory<-.jnew("ec/tstoolkit/sarima/SarimaModelBuilder")
  arima<-.jcall(factory, "Lec/tstoolkit/sarima/SarimaModel;", "createAirlineModel", as.integer(freq), as.double(th), as.double(bth))
  ...
  ts(data, start=startdate, frequency=freq)
}

jd_aggregate<-function(s, newfreq, mode="Sum"){
  jd_s<-ts_r2jd(s)
